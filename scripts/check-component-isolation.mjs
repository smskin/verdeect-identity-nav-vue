 ...
 *
 * `CrossServiceNav.vue` — чтение псевдоэлемента собственного узла ради
 * проверки, нарисована ли иконка. `UserMenu.vue` — слушатели закрытия меню,
 * живущие только пока меню раскрыто. Правка перечня обязана сопровождаться
 * доводом здесь же: иначе он перестанет что-либо значить.
 */
const DOCUMENT_ALLOWED = [
    `${CATALOG}/CrossServiceNav.vue`,
